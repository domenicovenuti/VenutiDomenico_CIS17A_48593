...
struct Grid { //The Board Game Grid Structure, have some parameters as rows, cols and an Dinamic array of Structure named Position for each position in the Grid (Nested Structures).
	int rows = 0;
	int cols = 0;
						...
						if (control3 == 0) {
							...
							while (control3 > 0) {
								...
								if (won > 0) {
									...
									showScores(1, score, grid.cols, players[player - 1]);
									won = 0;
									cout << endl;
									cout << "\t\tPress Enter to Go to the Main Menu...";
									getchar();
...
Grid createGrid(int rows, int cols, int* positions) {
	...
	*positions = rows * cols;
	...
	for (int x = 0; x < rows; x++) { //Initialize all Positions in the Board Game Grid as 0 (No movement made yet by any player).
		...
		if (value > 0) {
			for (int y = col; y < col + 4; y++) {
				for (int pos2 = 0; pos2 < positions; pos2++) {
					if (grid.position[pos2].x == row && grid.position[pos2].y == y && grid.position[pos2].value == value) {
						control1++;
					}
				}
			}
	...
	for (int pos1 = 0; pos1 < positions; pos1++) {
		...
		if (value > 0) {
			for (int x = row; x > row - 4; x--) {
				for (int pos2 = 0; pos2 < positions; pos2++) {
					if (grid.position[pos2].x == x && grid.position[pos2].y == col && grid.position[pos2].value == value) {
						control1++;
					}
				}
			}
	...
	for (int pos1 = 0; pos1 < positions; pos1++) {		
		...
		if (value > 0) {
			...
			for (int x = row; x > row - 4; x--) {
				...
				for (int pos2 = 0; pos2 < positions; pos2++) {					
					if (grid.position[pos2].x == x && grid.position[pos2].y == y && grid.position[pos2].value == value) {
						control1++;
					}
				}				
			}
